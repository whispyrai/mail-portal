...
- `npm test`: 1,082/1,082 passing after the Mailbox change-feed contract, migration and 21-trigger matrix, baseline/replay/future-cursor coverage, mounted authorization ordering, in-flight revocation, exact cache invalidation, browser lifecycle, bounded backoff, local-storage failure, and non-blocking revoked-route exit regressions.
- `npx tsc -b`: passing.
...
- Files is a separate 25.45 kB raw and 6.98 kB gzip route chunk. Initial mailbox JavaScript is 91.83 kB raw and 25.88 kB gzip after the persistent change-feed reconciler. Today is 26.73 kB raw and 8.12 kB gzip; Search is 27.76 kB raw and 10.04 kB gzip; EmailPanel is deferred at 78.98 kB raw and 23.21 kB gzip. ComposeEmail is deferred at 497.07 kB raw and 155.36 kB gzip; ComposeAiAssistant remains separate at 14.29 kB raw and 5.64 kB gzip; InboxTriageReview remains separate at 17.34 kB raw and 6.14 kB gzip.
...
The reviewed permission-aware change feed is checkpointed at `3fb8aca`. Build the first consumer above it next: a standalone mail-derived People and relationship workspace that remains Mailbox-local and has no Whispyr or WiserChat system coupling. Semantic indexing, Automation Rules, hibernating WebSocket notifications, and persisted offline mail remain later bounded slices. Visual QA still requires explicit approval. The standards pass remains pending explicit scope confirmation.
