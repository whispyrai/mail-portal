...
- The permission-aware Mailbox change feed is live. Migration 23 atomically records content-free ordered changes for Messages, attachments, folders, labels, Message-label relationships, deliveries, and delivery attempts. A strict authenticated route supports baseline and bounded replay, rechecks live access after storage work, and rejects malformed or future cursors. The Mailbox route owns one cursor-only reconciler with visible/hidden cadence, bounded backoff, offline pause, stale-response protection, exact non-AI cache invalidation, and immediate cache purge plus route exit on revoked access.
- Mail-derived People is live as a Mailbox-local deterministic workspace. Migration 24 records exact normalized external identities and Message participant evidence without inferred companies, merged aliases, sentiment, CRM state, or external enrichment. Live inbound, provider-accepted outbound, and admin import project inside authoritative Message transactions; bounded newest-first backfill and content-free change replay reconcile moves, Trash, restore, delete, and concurrent creation. Strict list, detail, and timeline routes authorize Personal owners and current Shared members, recheck access after work, and expose only bounded observed metadata. The adaptive browser workspace provides search, stable sorting and pagination, exact Conversation and file evidence, Compose, Copy address, and exact Message navigation. Unicode pagination has one SQLite and JavaScript order, and any direct 403 replaces the complete People tree before cache purge and route exit.
...
- `npm test`: 1,126/1,126 passing after deterministic People contracts, migration 24, direct projection, sender-name capture, bounded backfill, replay repair, Unicode-stable cursor pagination, mounted owner/member authorization, administrator/nonmember denial, in-flight revocation, adaptive browser states, exact Message links, Compose authority, change-feed invalidation, and synchronous revoked-content suppression regressions.
- `npx tsc -b`: passing.
...
- People is a separate 33.63 kB raw and 8.42 kB gzip route chunk. Files is 25.51 kB raw and 7.00 kB gzip. Initial mailbox JavaScript is 91.50 kB raw and 25.33 kB gzip. Today is 26.79 kB raw and 8.14 kB gzip; Search is 27.82 kB raw and 10.06 kB gzip; EmailPanel is deferred at 79.05 kB raw and 23.23 kB gzip. ComposeEmail is deferred at 497.17 kB raw and 155.42 kB gzip; ComposeAiAssistant remains separate at 14.35 kB raw and 5.67 kB gzip; InboxTriageReview remains separate at 17.40 kB raw and 6.16 kB gzip.
...
Checkpoint the reviewed deterministic People workspace, then build its manual cited AI relationship brief with actor-private caching, exact source fingerprints, cheap-tier inference, existing cost gates, post-work access revalidation, and no automatic mail mutation. Semantic indexing, Automation Rules, hibernating WebSocket notifications, and persisted offline mail remain later bounded slices. Visual QA still requires explicit approval. The standards pass remains pending explicit scope confirmation.
