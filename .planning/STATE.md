...
- The manual cited AI Relationship Brief is live inside People. It reads only bounded authored Message evidence after the deterministic projector is current, strips quoted reply and forward tails before side attribution, and returns only server-proven topics, open questions, explicit commitments, important Conversations, and one reviewed next-step suggestion. Generation is explicit, cheap-tier, actor-private, source-fingerprinted, durable-claim fenced, timeout bounded, and governed by the existing $25 alert and $50 review gate. It never drafts, sends, mutates, polls, or enters deterministic People invalidation.
