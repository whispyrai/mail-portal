...
type EmailDeletionArtifacts = {
  ...
  bodyObjects: EmailBodyObjectData[];
};
...
export class MailboxDO extends DurableObject<Env> {
  ...
  #collectEmailDeletionArtifacts(id: string): EmailDeletionArtifacts {
    const attachments = this.db
      ...
      .all();
    const bodyObjects = [
      ...this.ctx.storage.sql.exec<EmailBodyObjectData>(
        `SELECT id, email_id, part_index, content_type, charset, r2_key, byte_length
				 FROM email_body_objects WHERE email_id = ?1 ORDER BY part_index ASC`,
        id,
      ),
    ];
    return { attachments, bodyObjects };
  }

  #stageEmailDeletion(id: string, artifacts: EmailDeletionArtifacts): void {
    for (const attachment of artifacts.attachments) {
      this.ctx.storage.sql.exec(
        `INSERT OR IGNORE INTO r2_deletion_outbox (r2_key, email_id)
				 VALUES (?1, ?2)`,
        attachment.r2_key ??
    ...
    for (const bodyObject of artifacts.bodyObjects) {
      this.ctx.storage.sql.exec(
        `INSERT OR IGNORE INTO r2_deletion_outbox (r2_key, email_id)
				 VALUES (?1, ?2)`,
        bodyObject.r2_key,
    ...
    this.ctx.storage.sql.exec(
      `DELETE FROM inbound_terminal_failures WHERE id = ?1`,
      id,
    );
  }

  async #scheduleR2Deletion(): Promise<void> {
    const currentAlarm = await this.ctx.storage.getAlarm();
    if (currentAlarm === null || currentAlarm > Date.now() + 100) {
  ...
  async deleteEmail(id: string) {
    ...
    await this.#scheduleR2Deletion();

    const artifacts = this.ctx.storage.transactionSync(() => {
      const deletionArtifacts = this.#collectEmailDeletionArtifacts(id);
      this.#stageEmailDeletion(id, deletionArtifacts);
      this.db.delete(schema.emails).where(eq(schema.emails.id, id)).run();
      return deletionArtifacts;
    });
    return artifacts;
  }
  ...
  async claimInboundPush(id: string): Promise<boolean> {
    return this.ctx.storage.transactionSync(() => {
      ...
      this.ctx.storage.sql.exec(
        `INSERT INTO inbound_push_claims (id) VALUES (?1)`,
        id,
      );
      return true;
    });
  ...
  async deleteFolder(id: string) {
    ...
    const hasEmails =
      this.db
        ...
        .limit(1)
        .get() !== undefined;
    if (hasEmails) await this.#scheduleR2Deletion();

    const deletedEmailCount = this.ctx.storage.transactionSync(() => {
      const emailIds = this.db
        .select({ id: schema.emails.id })
        .from(schema.emails)
        .where(eq(schema.emails.folder_id, id))
        .all();
      for (const email of emailIds) {
        this.#stageEmailDeletion(
          email.id,
          this.#collectEmailDeletionArtifacts(email.id),
        );
      }
      this.db.delete(schema.folders).where(eq(schema.folders.id, id)).run();
      return emailIds.length;
    });
    console.log("[mail-cleanup] folder deletion staged", {
      deletedEmailCount,
      folderId: id,
  ...
  async createEmail(
    ...
    status: "deleted" | "duplicate" | "stored" | "terminal";
  }> {
    ...
    const result: {
      ...
      const terminal =
        [
          ...this.ctx.storage.sql.exec(
            `SELECT 1 AS found FROM inbound_terminal_failures WHERE id = ?1 LIMIT 1`,
            email.id,
          ),
        ].length > 0;
      if (terminal && !allowTerminalRecovery) return { status: "terminal" };
      if (terminal) {
        this.ctx.storage.sql.exec(
          `DELETE FROM inbound_terminal_failures WHERE id = ?1`,
          email.id,
      ...
      for (let index = 0; index < attachments.length; index += 10) {
        this.db
          ...
          .values(attachments.slice(index, index + 10))
          .run();
      }
      for (const bodyObject of bodyObjects) {
        this.ctx.storage.sql.exec(
          ...
          bodyObject.id,
          bodyObject.email_id,
          bodyObject.part_index,
          bodyObject.content_type,
          bodyObject.charset,
          bodyObject.r2_key,
          bodyObject.byte_length,
        );
      }
      this.ctx.storage.sql.exec(
        `DELETE FROM inbound_terminal_failures WHERE id = ?1`,
  ...
  async alarm(): Promise<void> {
    const deletionRows = [
      ...this.ctx.storage.sql.exec<{
        ...
        `SELECT r2_key, attempts FROM r2_deletion_outbox
				 WHERE next_attempt_at <= datetime('now')
				 ORDER BY created_at ASC LIMIT 100`,
      ),
    ];
    if (deletionRows.length > 0) {
      const keys = deletionRows.map((row) => row.r2_key);
      try {
        await this.env.BUCKET.delete(keys);
        this.ctx.storage.transactionSync(() => {
          for (const key of keys) {
            this.ctx.storage.sql.exec(
              `DELETE FROM r2_deletion_outbox WHERE r2_key = ?1`,
              key,
            );
          }
        });
        console.log("[mail-cleanup] R2 deletion batch completed", {
          count: keys.length,
    ...
    const cleanupPending =
      [
        ...this.ctx.storage.sql.exec(
          `SELECT 1 AS found FROM r2_deletion_outbox LIMIT 1`,
        ),
      ].length > 0;
    const queue = (await this.ctx.storage.get<string[]>(BULK_QUEUE_KEY)) ?? [];
    if (queue.length === 0) {
      if (cleanupPending) await this.ctx.storage.setAlarm(Date.now() + 60_000);
      return;
    }
    ...
    if (!job || job.status === "done" || job.cursor >= rows.length) {
      ...
      queue.shift();
      await this.ctx.storage.put(BULK_QUEUE_KEY, queue);
      if (queue.length > 0 || cleanupPending) {
        await this.ctx.storage.setAlarm(
          Date.now() + (queue.length > 0 ? 100 : 60_000),
        );
      }
      return;
    }
    ...
    if (queue.length > 0) {
      const delay =
        BULK_MIN_DELAY_MS + Math.floor(Math.random() * BULK_MAX_JITTER_MS);
