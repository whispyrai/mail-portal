...
test("a malformed success response is unknown because SES may have accepted it", () => {
	assert.deepEqual(
		classifySesOutcome({ kind: "invalid_success_response" }),
		{
			kind: "unknown",
			automaticRetry: false,
...
test("explicit SES rejections distinguish retryable and permanent failures", () => {
	assert.equal(
		classifySesOutcome({ kind: "http_error", status: 429 }).automaticRetry,
		true,
	);
	assert.equal(
		classifySesOutcome({ kind: "http_error", status: 503 }).automaticRetry,
		true,
	);
	assert.equal(
...
test("unclaimed queued or retrying mail can be cancelled", () => {
	...
	for (const status of ["sending", "sent", "bounced", "failed", "unknown", "cancelled"] as const) {
...
test("source draft survives until SES acceptance is confirmed", () => {
	for (const status of ["queued", "sending", "retrying", "failed", "unknown", "cancelled"] as const) {
...
test("unknown delivery requires an explicit force-retry transition", () => {
	assert.equal(
		transitionDelivery("unknown", "force_retry_unknown"),
		"queued",
	);
	assert.throws(
