...
test("importParsedEmail preserves metadata, attachments, threads, and idempotency", async () => {
	...
	const imported = await importParsedEmail({ bucket, mailbox }, parsed, "archive");
	const duplicate = await importParsedEmail({ bucket, mailbox }, parsed, "archive");
	const expectedThreadId = await deriveImportId({ messageId: "<root@zoho.example>" });
...
test("importParsedEmail cleans partial objects and retries with stable attachment ids", async () => {
	...
	const imported = await importParsedEmail({ bucket, mailbox }, parsed, "inbox");
